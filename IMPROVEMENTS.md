...
- `logs/` — 排除实验运行产生的日志目录
- `dataset/*.csv` — 排除大型数据集文件（仅保留 `ETTm2.csv`）
- `!dataset/ETTm2.csv` — 例外：保留内置的 ETTm2 数据集

这避免了将数 GB 的数据集文件和日志意外提交到 Git 仓库。
...
| 配置文件 | 1 (`requirements.txt`) | 1 (`.gitignore`) | 14 | 0 |
| 文档 | 1 (`IMPROVEMENTS.md`) | 2 (`README.md`, `train.py`, `dataset.py`) | ~250 | ~60 |
| **合计** | **4** | **4** | **~443** | **~60** |
